...
print(f"Loaded te data: {df_te.shape}")
print(f"Year range: {df_te['Year'].min()} - {df_te['Year'].max()}")

# Validate required columns exist
required_te_cols = ['Player', 'Year', 'Tm', 'PPR']
missing_cols = [col for col in required_te_cols if col not in df_te.columns]
if missing_cols:
    raise ValueError(f"Missing required columns in te data: {missing_cols}")
...
if available_rz_cols:
    df_te = df_te.merge(df_rz_rec[rz_merge_cols + available_rz_cols], on=rz_merge_cols, how='left')
    print(f"Merged red zone data: {available_rz_cols}")

# Calculate target share safely
if 'Tgt' in df_te.columns and 'Team_Tgt' in df_te.columns:
    df_te['Tgt_share'] = df_te['Tgt'] / df_te['Team_Tgt'].replace(0, np.nan)
    df_te['Tgt_share'] = df_te['Tgt_share'].fillna(0)
...
if 'PPR' in df_qb.columns:
    ...
    df_te = df_te.merge(df_qb_clean, how='left', left_on=['Tm', 'Year'], right_on=['qb_Tm', 'qb_Year'])
    df_te.drop(['qb_Tm', 'qb_Year'], axis=1, inplace=True)
...
if not df_adv_rec.empty:
    ...
    df_te = df_te.merge(df_adv_clean, on=['Player', 'Year'], how='left')
    print("Merged advanced receiving data")

# Calculate years played (excluding current year)
df_te = df_te.sort_values(['Player', 'Year']).reset_index(drop=True)
df_te['Career_Games'] = df_te.groupby('Player').cumcount() + 1
df_te['Years_Experience'] = df_te.groupby('Player')['Year'].transform(lambda x: x - x.min())
...
drop_cols = [
    ...
    'Pos_Rank', 'PosRank_prev', 'Rk', 'qb_Tm', 'qb_Year', 'DKPt', 'FDPt'  # Added qb columns to drop list
]
cols_to_drop = [col for col in drop_cols if col in df_te.columns]
if cols_to_drop:
    df_te.drop(columns=cols_to_drop, inplace=True)
    print(f"Dropped unnecessary columns: {len(cols_to_drop)}")

# Create shifted features for previous and 2 years ago
print("Creating lagged features...")
id_cols = ['Player', 'Year', 'Tm', 'Pos'] + [col for col in df_te.columns if any(x in col.lower() for x in ['top10', 'rank'])]
shift_cols = [col for col in df_te.columns if col not in id_cols and col != 'PPR']

# Memory efficient feature creation
shifted_features = {}
for col in shift_cols:
    if col in df_te.columns:
        shifted_features[f'{col}_prev'] = df_te.groupby('Player')[col].shift(1)
        shifted_features[f'{col}_2yr_prev'] = df_te.groupby('Player')[col].shift(2)

# Add shifted features to dataframe
df_te = pd.concat([df_te, pd.DataFrame(shifted_features, index=df_te.index)], axis=1)
...
if 'Drop' in df_te.columns and 'Tgt' in df_te.columns:
    efficiency_features['Drop_Rate'] = df_te['Drop'] / df_te['Tgt'].replace(0, np.nan)

# Reception efficiency  
if 'Rec' in df_te.columns and 'Tgt' in df_te.columns:
    efficiency_features['Catch_Rate'] = df_te['Rec'] / df_te['Tgt'].replace(0, np.nan)

# Yards per target efficiency
if 'Yds_rec' in df_te.columns and 'Tgt' in df_te.columns:
    efficiency_features['Yds_per_Tgt'] = df_te['Yds_rec'] / df_te['Tgt'].replace(0, np.nan)

# TD efficiency  
if 'TD_rec' in df_te.columns and 'Tgt' in df_te.columns:
    efficiency_features['TD_per_Tgt'] = df_te['TD_rec'] / df_te['Tgt'].replace(0, np.nan)

# Air yards efficiency
if 'Air' in df_te.columns and 'Tgt' in df_te.columns:
    efficiency_features['Air_per_Tgt'] = df_te['Air'] / df_te['Tgt'].replace(0, np.nan)

# YAC efficiency
if 'YAC' in df_te.columns and 'Rec' in df_te.columns:
    efficiency_features['YAC_per_Rec'] = df_te['YAC'] / df_te['Rec'].replace(0, np.nan)

# Add efficiency features
if efficiency_features:
    efficiency_df = pd.DataFrame(efficiency_features, index=df_te.index).fillna(0)
    df_te = pd.concat([df_te, efficiency_df], axis=1)
    ...
    existing_to_remove = [col for col in volume_stats_to_remove if col in df_te.columns]
    if existing_to_remove:
        df_te.drop(columns=existing_to_remove, inplace=True)
...
hist_cols = [col for col in hist_cols if col in df_te.columns]
...
for col in hist_cols:
    historical_features[f'{col}_2yr_avg'] = df_te.groupby('Player')[col].transform(safe_2yr_avg)

df_te = pd.concat([df_te, pd.DataFrame(historical_features, index=df_te.index)], axis=1)
print(f"Created {len(historical_features)} historical average features")

# Fill missing values strategically
print("Handling missing values...")
numeric_cols = df_te.select_dtypes(include=[np.number]).columns
categorical_cols = df_te.select_dtypes(include=['object']).columns
...
current_year = 2024
df_te_current = df_te[df_te['Year'] == current_year].copy()

# Fix #2: Don't overteite existing 2024 PPR data
if not df_te_current.empty:
    df_te_current['PPR'] = df_te_current['PPR'].fillna(0)  # Only fill NaN values
    print(f"Found {len(df_te_current)} players with 2024 data")
else:
    print("No 2024 data found for predictions")

# Prepare training data
df_train = df_te[df_te['Year'] < current_year].copy()
...
if not df_te_current.empty:
    ...
    veteran_data = df_te[df_te.groupby('Player')['Year'].transform('count') >= 3]
    league_avg_features = {}
    ...
    for pid, group in df_te.groupby('Player'):
    ...
    df_te_current = df_te_current.drop(columns=[c for c in historical_features if c in df_te_current.columns], errors='ignore')
    df_te_current = df_te_current.merge(weighted_hist_df, left_index=True, right_index=True, how='left')
    
    # Ensure all required features are present
    missing_features = [col for col in feature_cols if col not in df_te_current.columns]
    for col in missing_features:
        df_te_current[col] = df_train[col].median()
...
df_te_filtered = df_train[['Player', 'Year', 'PPR'] + feature_cols].copy()

# Check if we have data for the current year in our dataset
years_available = sorted(df_te_filtered['Year'].unique())
...
if current_year not in years_available:
    ...
    train_data = df_te_filtered[df_te_filtered['Year'] != eval_year].copy()
    test_data = df_te_filtered[df_te_filtered['Year'] == eval_year].copy()
    ...
    s_result, s_r2, s_mae, s_te_coef, s_best, test = ff.evaluate_model(df_te_filtered, eval_year, 'PPR', 'SVR')
else:
    # Use current year if it exists in data
    s_result, s_r2, s_mae, s_te_coef, s_best, test = ff.evaluate_model(df_te_filtered, current_year, 'PPR', 'SVR')
...
if not df_te_current.empty:
    ...
    df_te_current_aligned = df_te_current.reindex(columns=trained_features, fill_value=0)
    df_te_current_aligned = df_te_current_aligned.fillna(0)
    
    # Make predictions
    predictions = test.predict(df_te_current_aligned)
    df_te_current['PPR_prediction_2025'] = predictions
    ...
    available_top_features = [f for f in top_features if f in df_te_current_aligned.columns]
    
    # Prepare output - handle duplicate players first
    print("Preparing output data...")
    
    # Reset index to avoid duplicate index issues
    df_te_current_reset = df_te_current.reset_index()
    df_te_current_aligned_reset = df_te_current_aligned.reset_index()
    
    # Remove duplicates early (keep first occurrence)
    df_te_current_unique = df_te_current_reset.drop_duplicates(subset=['Player'], keep='first')
    df_te_current_aligned_unique = df_te_current_aligned_reset.drop_duplicates(subset=['Player'], keep='first')
    
    # Set Player as index again for alignment
    df_te_current_unique = df_te_current_unique.set_index('Player')
    df_te_current_aligned_unique = df_te_current_aligned_unique.set_index('Player')
    
    # Sort by predictions
    df_te_current_unique = df_te_current_unique.sort_values('PPR_prediction_2025', ascending=False)
    
    # Create comprehensive output using unique data
    df_output = df_te_current_unique[['PPR_prediction_2025']].copy()
    
    # Add metadata
    if 'experience_level' in df_te_current_unique.columns:
        df_output['experience_level'] = df_te_current_unique['experience_level']
    if 'years_available' in df_te_current_unique.columns:
        df_output['years_available'] = df_te_current_unique['years_available']
    
    # Add top features safely
    for feature in available_top_features[:10]:
        if feature in df_te_current_aligned_unique.columns:
            # Align indices properly
            feature_values = df_te_current_aligned_unique[feature]
    ...
    with pd.ExcelWriter(os.path.join(result_dir, 'te_model_report.xlsx')) as teiter:
        metrics_df.to_excel(teiter, sheet_name='Model_Metrics', index=False)
        quality_df.to_excel(teiter, sheet_name='Data_Quality', index=False)
        coef_df.head(25).to_excel(teiter, sheet_name='Feature_Importance', index=False)
        df_output.to_excel(teiter, sheet_name='Predictions', index=False)
        
        # CV results if available
        if cv_results:
            pd.DataFrame(cv_results).to_excel(teiter, sheet_name='Cross_Validation', index=False)
    
    print(f"Predictions completed for {len(df_output)} players")
    print(f"Results saved to: {os.path.join(result_dir, 'te_model_report.xlsx')}")
